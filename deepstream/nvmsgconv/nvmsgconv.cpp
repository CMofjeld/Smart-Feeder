...
generate_object_object (NvDsMsg2pCtx *ctx, NvDsEventMsgMeta *meta)
{
  ...
  switch (meta->objType) {
    ...
    case NVDS_OBJECT_TYPE_FACE_EXT:
      ...
      json_object_set_object_member (objectObj, "face", jobject);
      break;
    case NVDS_OBJECT_TYPE_BIRD:
      jobject = json_object_new ();
      if (meta->extMsgSize) {
        NvDsBirdObject *dsObj = (NvDsBirdObject *) meta->extMsg;
        if (dsObj) {
          json_object_set_string_member (jobject, "species", dsObj->species);
        }
      } else {
        json_object_set_string_member (jobject, "species", "");
      }
      json_object_set_object_member (objectObj, "bird", jobject);
      break;
